...
function getScoreLabel(score: number, total: number): [string, string] {
...
export function QuizMode({ questions, onSignupTrigger }: QuizModeProps) {
  ...
  // ── Results screen ─────────────────────────────────────────────────────────
  if (state === "done") {
    const [scoreLabel, scoreSub] = getScoreLabel(score, questions.length);
      ...
      <div className="animate-fade-up flex flex-col gap-5">
        ...
        <div className="card-purple p-6 text-center">
          <div className="animate-float inline-block mb-4">
            <DexMascot size={64} mood={score >= 7 ? "excited" : score >= 5 ? "happy" : "thinking"} animate />
          </div>
        ...
        {weak.length > 0 && (
          <div className="card p-5">
            <p className="font-bold text-sm text-ink mb-3">
              📌 Areas to review
            </p>
        ...
        <div className="card-subtle p-5">
          ...
          <p className="text-sm text-muted leading-relaxed">
            {pct >= 80
              ? "Excellent retention! Try switching the explanation to Expert level and re-reading for deeper nuance."
              : pct >= 60
              ? "Good foundation. Re-read the key takeaways, then try the quiz again to solidify weak spots."
              : "Head back to the Explain tab and try switching to Age 12 mode for a clearer breakdown, then retake the quiz."}
          </p>
        </div>
